...
exort.exe on the HITRAN-2024 native-gas line list (the default since
2026-09-23; before that the suite baselined against HITRAN-2016 — see
EXORT_H16_N68vN84_GRID.md for the proof that exort-on-h16 reproduces the
retired n68equiv physics).

HITRAN-2016 side-path
---------------------
  python run_regression.py --exort h16      # build+run exort.exe with the
                                            # HITRAN-2016 native k-files swapped in

--exort h16 temporarily swaps the four native-gas (h2o/co2/ch4/c2h6) filename
strings in src.exort/kabs.F90 to their HITRAN-2016 counterparts (H2O = the
T-index-fixed table), rebuilds, runs, then restores kabs.F90 so the working tree
is left unchanged. It compares against the committed (h24) baselines — a
line-list comparison, not a passing gate. It cannot be combined with
--generate-baselines.
...
# --- src.exort build + HITRAN-2016 side-path (--exort h16) -------------------
# exort.exe is the primary regression target. kabs.F90 commits the native gases
# (h2o/co2/ch4/c2h6) to HITRAN-2024, so the default build needs no edit. The
# optional --exort h16 mode temporarily swaps those four k-filename strings to
# their HITRAN-2016 counterparts, rebuilds, runs, then restores the file — for
# comparing the older line list against the baselines.
EXORT_EXE = "exort.exe"
BUILD_DIR = os.path.join(REPO, "build")
KABS_FILE = os.path.join(REPO, "source", "src.exort", "kabs.F90")

# HITRAN-2016 filename  ->  HITRAN-2024 counterpart (as committed in kabs.F90).
...
def build_exort(env, hitran):
    ...
    'h24' is the committed default — kabs.F90 already points the native gases at
    HITRAN-2024, so the build proceeds unmodified. For 'h16' the four native-gas
    k-filename strings in src.exort/kabs.F90 are temporarily swapped to their
    HITRAN-2016 names before the build. The caller is responsible for restoring
...
def main():
    ...
    ap.add_argument("--exort", choices=["h16", "h24"], default=None,
                    help="select the native-gas HITRAN vintage for the exort "
                         "build. Default (omitted) = h24, the committed/baselined "
                         "line list. h16 temporarily swaps in the HITRAN-2016 "
                         "native k-files for a comparison run against the h24 "
                         "baselines; cannot be combined with "
                         "--generate-baselines.")
    args = ap.parse_args()

    if args.exort == "h16" and args.generate_baselines:
        sys.exit("ERROR: --exort h16 and --generate-baselines are mutually "
                 "exclusive. The committed baselines are the default HITRAN-2024 "
                 "build; generate baselines on the default (h24) build.")
    ...
    # exist, then build exort.exe for the requested HITRAN vintage (default h24,
    # already committed in kabs.F90; --exort h16 swaps in the HITRAN-2016 native
    # k-files for the build, restored afterward).
    hitran = args.exort or "h24"
    missing_stars = set()
    for c in cases:
    ...
    try:
        ...
        if saved_nl is not None:
            with open(NAMELIST, "w") as fh:
                fh.write(saved_nl)
        # Restore kabs.F90 if --exort h16 swapped it.
    ...
    if args.generate_baselines:
        ...
        print(f"=== exort.exe (h16) vs h24 baselines "
              f"(rtol={args.rtol}, atol={args.atol}) ===")
    else:
        print(f"=== regression results: exort.exe (h24) "
              f"(rtol={args.rtol}, atol={args.atol}) ===")
