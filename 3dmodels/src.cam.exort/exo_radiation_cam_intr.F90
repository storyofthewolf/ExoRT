...
!
! CONTRACT: the exo_radiation_tend argument list is the primary linkage
! between ExoRT and CESM/ExoCAM and must NOT change. Internals are free
! to evolve; new physics inputs enter through aerad_driver's optional
! keyword tail, never through exo_radiation_tend's signature.
!
! Optional physics paths are compiled in per ExoCAM configuration via
! CPP macros in CAM_CONFIG_OPTS (-cppdefs):
!   -DEXORT_CO2CLD    CO2 ice cloud radiative effects. Requires the
!                     ExoCAM co2condense configuration: exo_condense_mod
!                     (CICEWP_CO2/REI_CO2 pbuf fields) and
!                     do_exo_condense_co2 in exoplanet_mod.
!   -DEXORT_CARMA     CARMA haze radiative effects. Requires the ExoCAM
!                     carma configuration: carma_exort_mod and the CARMA
!                     microphysics (carma_model_mod). Haze optics are
!                     loaded by ExoRT itself (initialize_hazeopts) from
!                     data/aerosol/; carma_exort_optics_init is NOT used.
!   -DEXORT_SRF_EMISS Surface thermal emissivity from the coupler
!                     (cam_in%srf_emiss). Requires the camsrfexch/coupler
!                     modifications (currently in the co2condense config).
! With no macros defined this module compiles against the mainline
! ExoCAM configurations, with H2O clouds only (like src.cam.n68equiv).
!
! In ALL cases ExoCAM's src.share/exoplanet_mod.F90 must provide the v2
! flags:  logical do_exo_clouds (.true. for 3-D), and do_exo_haze
! (recommended: do_exo_haze = do_carma_exort). The shared driver
! (exo_radiation_mod.F90) gates the cloud/haze kernels on them.
!
! Revision history
...
! July      2026, v2 port: keyword-tail driver calls; CO2 ice cloud,
!                 CARMA haze, and srf_emiss paths merged behind CPP flags
!
...
!
  ...
  use exoplanet_mod,    only: do_exo_rt_clearsky, exo_rad_step, do_exo_rt_spectral, &
                              ...
                              do_exo_haze
#ifdef EXORT_CO2CLD
  ! CO2 ice clouds require the ExoCAM co2condense configuration
  ! (exo_condense_mod provides the CICEWP_CO2/REI_CO2 pbuf fields and
  ! exoplanet_mod the flag). Enable with -cppdefs '-DEXORT_CO2CLD'.
  use exoplanet_mod,    only: do_exo_condense_co2
#endif
#ifdef EXORT_CARMA
  ! CARMA haze requires the ExoCAM carma configuration (carma_exort_mod +
  ! the CARMA microphysics). Enable with -cppdefs '-DEXORT_CARMA'.
  use exoplanet_mod,    only: do_carma_exort
  use carma_exort_mod,  only: carma_exort_get_mmr
...
#ifdef EXORT_CARMA
    ! The RT haze optics tables are compiled on (nelem_carma, nbin_carma)
    ! from radgrid; the CARMA microphysics element/bin grid must match.
    if (NELEM /= nelem_carma .or. NBIN /= nbin_carma) then
      call endrun('exo_radiation_init: CARMA NELEM/NBIN do not match radgrid nelem_carma/nbin_carma')
    endif
#endif
...
#ifdef EXORT_CO2CLD
    real(r8), pointer, dimension(:,:) :: cicewp_co2  ! in-cloud CO2 ice water path (g/m2)
    real(r8), pointer, dimension(:,:) :: rei_co2     ! CO2 ice effective particle radius (microns)
    ! zero-opacity fallbacks so the driver call is unconditional when
    ! do_exo_condense_co2 is off in a build that carries EXORT_CO2CLD
    real(r8), dimension(pcols,pver), target :: cicewp_co2_off
    real(r8), dimension(pcols,pver), target :: rei_co2_off
#endif
#ifdef EXORT_CARMA
    real(r8), dimension(pcols,pver,NELEM,NBIN) :: carmammr  ! CARMA binwise constituent mass mixing ratios [kg/kg]
#endif
...
!
    ...
    if (do_exo_condense_co2) then
      ...
      cicewp_co2_off(:,:) = 0.0_r8
      rei_co2_off(:,:)    = 10.0_r8   ! any in-range radius; zero ice path -> zero opacity
      cicewp_co2 => cicewp_co2_off
      rei_co2    => rei_co2_off
    endif
#endif
    ...
    if (do_exo_rad) then
...
#ifdef EXORT_CARMA
      ! CARMA binwise constituent mass mixing ratios for the haze RT path
      ...
      if (do_exo_rt_clearsky) then
        ...
        do i = 1, ncol
          ...
          ! (H2O clouds, CO2 ice clouds, CARMA haze) are simply omitted --
          ! absent arguments contribute zero opacity by contract. Surface
          ! emissivity is a surface property, not a cloud, so it is passed
          ! here too when the build carries it.
...
#ifdef EXORT_SRF_EMISS
                            , ext_srf_emiss=cam_in%srf_emiss(i) &
#endif
                            )
      ...
      do i = 1, ncol
        ...
        ! Full-sky call: condensed-phase inputs ride the optional keyword
        ! tail. H2O clouds are always passed (the driver gates them on
        ! do_exo_clouds); CO2 ice clouds, CARMA haze, and surface
        ! emissivity are compiled in only for ExoCAM configurations that
        ! provide them (-DEXORT_CO2CLD / -DEXORT_CARMA / -DEXORT_SRF_EMISS).
        call aerad_driver(h2ommr(i,:), co2mmr(i,:), &
                          ch4mmr(i,:), c2h6mmr(i,:), &
                          nh3mmr(i,:), commr(i,:), &
                          h2mmr(i,:),  n2mmr(i,:), o3mmr(i,:), o2mmr(i,:), &
...
#ifdef EXORT_CO2CLD
                          , ext_cicewp_co2=cicewp_co2(i,:), ext_rei_co2=rei_co2(i,:) &
#endif
#ifdef EXORT_CARMA
                          , ext_carmammr=carmammr(i,:,:,:) &
#endif
#ifdef EXORT_SRF_EMISS
                          , ext_srf_emiss=cam_in%srf_emiss(i) &
#endif
