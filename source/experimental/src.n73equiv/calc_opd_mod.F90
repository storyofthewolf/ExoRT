...
!!write(*,*) "kmajor", ans_kmajor_gptvec
      ...
      !=====  interval 69:  =====!
      !=====  23809.5 - 25974.0 cm-1 
      sp=69
      call bilinear_interpK_grey(k69_grey_data, iH2O, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_h2o) 
      call bilinear_interpK_grey(k69_grey_data, iCO2, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_co2) 
      call bilinear_interpK_grey(k69_grey_data, iCH4, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_ch4)
      tau_grey(iH2O) = ans_kgrey_h2o * ugas(iH2O)
      tau_grey(iCO2) = ans_kgrey_co2 * ugas(iCO2)
      tau_grey(iCH4) = ans_kgrey_ch4 * ugas(iCH4)
      imajor = maxloc(tau_grey)
!write(*,*) ik, sp, gas_name(imajor), tau_grey, ans_kgrey_h2o, ans_kgrey_co2, ans_kgrey_ch4
      ! major gas (k-distribution)
!!write(*,*) "kmajor", ans_kmajor_gptvec
      call bilinear_interpK_8gpt_major_gptvec(k69_major_data, imajor(1), pressure, p_ref_index, t_kgas, t_ref_index, ans_kmajor_gptvec) 
      do ig = 1,ngauss_pts(sp)      
        itl = itl + 1         
        tau_gas(itl,ik) = ans_kmajor_gptvec(ig)*ugas(imajor(1)) + (tau_grey(iH2O) + tau_grey(iCO2) + tau_grey(iCH4) - tau_grey(imajor(1)))
      enddo   
      ...
      !=====  interval 70:  =====!
      !=====  25974.0 - 28985.5 cm-1 
      sp=70
      call bilinear_interpK_grey(k70_grey_data, iH2O, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_h2o) 
      call bilinear_interpK_grey(k70_grey_data, iCO2, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_co2) 
      call bilinear_interpK_grey(k70_grey_data, iCH4, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_ch4)
      tau_grey(iH2O) = ans_kgrey_h2o * ugas(iH2O)
      tau_grey(iCO2) = ans_kgrey_co2 * ugas(iCO2)
      tau_grey(iCH4) = ans_kgrey_ch4 * ugas(iCH4)
      imajor = maxloc(tau_grey)
!write(*,*) ik, sp, gas_name(imajor), tau_grey, ans_kgrey_h2o, ans_kgrey_co2, ans_kgrey_ch4
      ! major gas (k-distribution)
!!write(*,*) "kmajor", ans_kmajor_gptvec
      call bilinear_interpK_8gpt_major_gptvec(k70_major_data, imajor(1), pressure, p_ref_index, t_kgas, t_ref_index, ans_kmajor_gptvec) 
      do ig = 1,ngauss_pts(sp)      
        itl = itl + 1         
        tau_gas(itl,ik) = ans_kmajor_gptvec(ig)*ugas(imajor(1)) + (tau_grey(iH2O) + tau_grey(iCO2) + tau_grey(iCH4) - tau_grey(imajor(1)))
      enddo   
      ...
      !=====  25974.0 - 28985.5 cm-1 
      sp=71
      call bilinear_interpK_grey(k71_grey_data, iH2O, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_h2o) 
      call bilinear_interpK_grey(k71_grey_data, iCO2, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_co2) 
      call bilinear_interpK_grey(k71_grey_data, iCH4, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_ch4)
      tau_grey(iH2O) = ans_kgrey_h2o * ugas(iH2O)
      tau_grey(iCO2) = ans_kgrey_co2 * ugas(iCO2)
      tau_grey(iCH4) = ans_kgrey_ch4 * ugas(iCH4)
      imajor = maxloc(tau_grey)
!write(*,*) ik, sp, gas_name(imajor), tau_grey, ans_kgrey_h2o, ans_kgrey_co2, ans_kgrey_ch4
      ! major gas (k-distribution)
!!write(*,*) "kmajor", ans_kmajor_gptvec
      call bilinear_interpK_8gpt_major_gptvec(k71_major_data, imajor(1), pressure, p_ref_index, t_kgas, t_ref_index, ans_kmajor_gptvec) 
      do ig = 1,ngauss_pts(sp)      
        itl = itl + 1         
        tau_gas(itl,ik) = ans_kmajor_gptvec(ig)*ugas(imajor(1)) + (tau_grey(iH2O) + tau_grey(iCO2) + tau_grey(iCH4) - tau_grey(imajor(1)))
      enddo   
      ...
      !=====  interval 72:  =====!
      !=====  28985.5 - 33333.3 cm-1 
      sp=72
      call bilinear_interpK_grey(k72_grey_data, iH2O, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_h2o) 
      call bilinear_interpK_grey(k72_grey_data, iCO2, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_co2) 
      call bilinear_interpK_grey(k72_grey_data, iCH4, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_ch4)
      tau_grey(iH2O) = ans_kgrey_h2o * ugas(iH2O)
      tau_grey(iCO2) = ans_kgrey_co2 * ugas(iCO2)
      tau_grey(iCH4) = ans_kgrey_ch4 * ugas(iCH4)
      imajor = maxloc(tau_grey)
!write(*,*) ik, sp, gas_name(imajor), tau_grey, ans_kgrey_h2o, ans_kgrey_co2, ans_kgrey_ch4
      ! major gas (k-distribution)
!!write(*,*) "kmajor", ans_kmajor_gptvec
      call bilinear_interpK_8gpt_major_gptvec(k72_major_data, imajor(1), pressure, p_ref_index, t_kgas, t_ref_index, ans_kmajor_gptvec) 
      do ig = 1,ngauss_pts(sp)      
        itl = itl + 1         
        tau_gas(itl,ik) = ans_kmajor_gptvec(ig)*ugas(imajor(1)) + (tau_grey(iH2O) + tau_grey(iCO2) + tau_grey(iCH4) - tau_grey(imajor(1)))
      enddo   
      ...
      !=====  33333.3 - 50000. cm-1 
      sp=73
      call bilinear_interpK_grey(k73_grey_data, iH2O, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_h2o) 
      call bilinear_interpK_grey(k73_grey_data, iCO2, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_co2) 
      call bilinear_interpK_grey(k73_grey_data, iCH4, pressure, p_ref_index, t_kgas, t_ref_index, ans_kgrey_ch4)
      tau_grey(iH2O) = ans_kgrey_h2o * ugas(iH2O)
      tau_grey(iCO2) = ans_kgrey_co2 * ugas(iCO2)
      tau_grey(iCH4) = ans_kgrey_ch4 * ugas(iCH4)
      imajor = maxloc(tau_grey)
!write(*,*) ik, sp, gas_name(imajor), tau_grey, ans_kgrey_h2o, ans_kgrey_co2, ans_kgrey_ch4
      ! major gas (k-distribution)
!!write(*,*) "kmajor", ans_kmajor_gptvec
      call bilinear_interpK_8gpt_major_gptvec(k73_major_data, imajor(1), pressure, p_ref_index, t_kgas, t_ref_index, ans_kmajor_gptvec) 
      do ig = 1,ngauss_pts(sp)      
        itl = itl + 1         
        tau_gas(itl,ik) = ans_kmajor_gptvec(ig)*ugas(imajor(1)) + (tau_grey(iH2O) + tau_grey(iCO2) + tau_grey(iCH4) - tau_grey(imajor(1)))
      enddo   
      ...
      !!  Do MT_CKD continuum           
      itc=0
      call interpH2Omtckd(kh2oself_mtckd, t_h2o_mtckd, t_ref_index_mtckd, ans_h2os)
      call interpH2Omtckd(kh2ofrgn_mtckd, t_h2o_mtckd, t_ref_index_mtckd, ans_h2of)
...
!        if (iw.gt.41) ans_h2os(iw) = 0.
 !       if (iw.gt.41) ans_h2of(iw) = 0.
        do ig=1, ngauss_pts(iw)
          itc=itc+1
!          tau_gas(itc,ik) = tau_gas(itc,ik) + (ans_h2os(iw)*amaH2O + ans_h2of(iw)*amaFRGN) * u_h2o
!          tau_gas(itc,ik) = tau_gas(itc,ik) + (ans_h2os(iw)*amaH2O)* u_h2o  ! add H2O self continuum
!          tau_gas(itc,ik) = tau_gas(itc,ik) + (ans_h2of(iw)*amaFRGN)* u_h2o ! add H2O FRGN continuum
         enddo
