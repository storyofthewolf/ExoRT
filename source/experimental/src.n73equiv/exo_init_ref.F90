...
!=============================================================
   ...
   !=====  interval 69:   =====!
   k69_major_data(iH2O,:,:,:) = k69_h2o(:,:,:) 
   k69_major_data(iCO2,:,:,:) = k69_co2(:,:,:)
   k69_major_data(iCH4,:,:,:) = k69_ch4(:,:,:)
   !=====  interval 70:   =====!
   k70_major_data(iH2O,:,:,:) = k70_h2o(:,:,:) 
   k70_major_data(iCO2,:,:,:) = k70_co2(:,:,:)
   k70_major_data(iCH4,:,:,:) = k70_ch4(:,:,:)
   !=====  interval 71:   =====!
   k71_major_data(iH2O,:,:,:) = k71_h2o(:,:,:) 
   k71_major_data(iCO2,:,:,:) = k71_co2(:,:,:)
   k71_major_data(iCH4,:,:,:) = k71_ch4(:,:,:)
   !=====  interval 72:   =====!
   k72_major_data(iH2O,:,:,:) = k72_h2o(:,:,:) 
   k72_major_data(iCO2,:,:,:) = k72_co2(:,:,:)
   k72_major_data(iCH4,:,:,:) = k72_ch4(:,:,:)
   !=====  interval 73:   =====!
   k73_major_data(iH2O,:,:,:) = k73_h2o(:,:,:) 
   k73_major_data(iCO2,:,:,:) = k73_co2(:,:,:)
   k73_major_data(iCH4,:,:,:) = k73_ch4(:,:,:)
...
!
   ...
   !=====  interval 69:   =====!
   k69_grey_data(:,:,:) = 0.0
   do ig=1, ngauss_pts(69)
     iq = iq + 1
     k69_grey_data(iH2O,:,:) = k69_grey_data(iH2O,:,:) + k69_h2o(ig,:,:) * g_weight(iq)
     k69_grey_data(iCO2,:,:) = k69_grey_data(iCO2,:,:) + k69_co2(ig,:,:) * g_weight(iq)
     k69_grey_data(iCH4,:,:) = k69_grey_data(iCH4,:,:) + k69_ch4(ig,:,:) * g_weight(iq)
   enddo
   !=====  interval 70:   =====!
   k70_grey_data(:,:,:) = 0.0
   do ig=1, ngauss_pts(70)
     iq = iq + 1
     k70_grey_data(iH2O,:,:) = k70_grey_data(iH2O,:,:) + k70_h2o(ig,:,:) * g_weight(iq)
     k70_grey_data(iCO2,:,:) = k70_grey_data(iCO2,:,:) + k70_co2(ig,:,:) * g_weight(iq)
     k70_grey_data(iCH4,:,:) = k70_grey_data(iCH4,:,:) + k70_ch4(ig,:,:) * g_weight(iq)
   enddo
   !=====  interval 71:   =====!
   k71_grey_data(:,:,:) = 0.0
   do ig=1, ngauss_pts(71)
     iq = iq + 1
     k71_grey_data(iH2O,:,:) = k71_grey_data(iH2O,:,:) + k71_h2o(ig,:,:) * g_weight(iq)
     k71_grey_data(iCO2,:,:) = k71_grey_data(iCO2,:,:) + k71_co2(ig,:,:) * g_weight(iq)
     k71_grey_data(iCH4,:,:) = k71_grey_data(iCH4,:,:) + k71_ch4(ig,:,:) * g_weight(iq)
   enddo
   !=====  interval 72:   =====!
   k72_grey_data(:,:,:) = 0.0
   do ig=1, ngauss_pts(72)
     iq = iq + 1
     k72_grey_data(iH2O,:,:) = k72_grey_data(iH2O,:,:) + k72_h2o(ig,:,:) * g_weight(iq)
     k72_grey_data(iCO2,:,:) = k72_grey_data(iCO2,:,:) + k72_co2(ig,:,:) * g_weight(iq)
     k72_grey_data(iCH4,:,:) = k72_grey_data(iCH4,:,:) + k72_ch4(ig,:,:) * g_weight(iq)
   enddo
   !=====  interval 73:   =====!
   k73_grey_data(:,:,:) = 0.0
   do ig=1, ngauss_pts(70)
     iq = iq + 1
     k73_grey_data(iH2O,:,:) = k73_grey_data(iH2O,:,:) + k73_h2o(ig,:,:) * g_weight(iq)
     k73_grey_data(iCO2,:,:) = k73_grey_data(iCO2,:,:) + k73_co2(ig,:,:) * g_weight(iq)
     k73_grey_data(iCH4,:,:) = k73_grey_data(iCH4,:,:) + k73_ch4(ig,:,:) * g_weight(iq)
   enddo
