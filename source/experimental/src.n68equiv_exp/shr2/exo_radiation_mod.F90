...
!
     ...
     real(r8), dimension(pverp,nelem,nbin) :: qcarmammr   ! carma cloud/aerosol mixing ratios
...
!
    ...
    ! call carma aerosol optical depths
    if(do_carma_rt) call calc_aeropd(qcarmammr, zlayer*100, ext_pdel, bext_aer_carma, tau_aer_carma, taueff_aer_carma)
    ...
    call rad_precalc(pmid/100.0, tmid, tint, swcut, tau_gas, tau_ray, &
                     ...
                     tau_aer_carma, singscat_aer_carma, asym_aer_carma, &
...
! 
    ...
    real(r8), intent(in), dimension(ntot_wavlnrng,pverp) ::  tau_aer_carma
    real(r8), intent(in), dimension(ntot_wavlnrng,pverp) ::  singscat_aer_carma
    real(r8), intent(in), dimension(ntot_wavlnrng,pverp) ::  asym_aer_carma
          ...
          if (do_exo_carma_rt) 
            do ie=1,nelem
              do ib=1,nbin
                ...
                w0_ig = w0_ig + singscat_aer_carma(ie,ib,iw) * tau_aer_carma(ie,ib,iw,k)
                g0_ig = g0_ig + asym_aer_carma(ie,ib,iw) * singscat_aer_carma(ie,ib,iw) * tau_aer_carma(ie,ib,iw,k)
              enddo
            enddo 
          endif
