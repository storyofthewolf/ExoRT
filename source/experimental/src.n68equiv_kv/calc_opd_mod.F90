...
write(*,*) "MTCKD ", t_h2os, t_ref_index_s, ans_h2os
        do iw = iwbeg,iwend     ! loop over bands      
        ! apply H2O self continuum between 500 and 1400, and 2100 to 3000 cm-1
         ...
         elseif(wavenum_mid(iw).gt.2100 .and. wavenum_mid(iw).lt.17000) then
         else
           ans_h2os(iw) = 0.0
         endif
          do ig=1, ngauss_pts(iw)
            itc=itc+1
            tau_gas(itc,ik) = tau_gas(itc,ik) +ans_h2os(iw)*u_h2o*(pmid(ik)/1013.)*h2ovmr           ! original
!            tau_gas(itc,ik) = tau_gas(itc,ik) + ans_h2os(iw)*u_h2o
!           tau_gas(itc,ik) = tau_gas(itc,ik) +ans_h2os(iw)*u_h2o*(273.15/temperature)*(h2ovap_press/1013.250)
!           tau_gas(itc,ik) = tau_gas(itc,ik) +ans_h2os(iw)*u_h2o*(h2ovap_press/1013.250)              
          enddo
        enddo    ! close band loop
      endif
