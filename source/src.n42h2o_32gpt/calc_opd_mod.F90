...
!
    ...
    do ik = 1,pverp 
      ...
      do ig = 1,ngauss_pts(1)      
        itl = itl + 1         
        call bilinear_interpK_32gpt(k01_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
      ...
      do ig = 1,ngauss_pts(2)    
        itl = itl + 1
        call bilinear_interpK_32gpt(k02_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(3)   
        itl = itl + 1
        call bilinear_interpK_32gpt(k03_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(4)          
        itl = itl + 1
        call bilinear_interpK_32gpt(k04_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col
      enddo   
      ...
      do ig = 1,ngauss_pts(5)   
        itl = itl + 1
        call bilinear_interpK_32gpt(k05_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(6)    
        itl = itl + 1
        call bilinear_interpK_32gpt(k06_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(7)		  
        itl = itl + 1
        call bilinear_interpK_32gpt(k07_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(8)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k08_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(9)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k09_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(10)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k10_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(11)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k11_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(12)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k12_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(13)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k13_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(14)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k14_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(15)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k15_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(16)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k16_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(17)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k17_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(18)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k18_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(19)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k19_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(20)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k20_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(21)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k21_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(22)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k22_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(23)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k23_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(24)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k24_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(25)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k25_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(26)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k26_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(27)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k27_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(28)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k28_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(29)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k29_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(30)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k30_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(31)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k31_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(32)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k32_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(33)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k33_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(34)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k34_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(35)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k35_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(36)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k36_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(37)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k37_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(38)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k38_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(39)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k39_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(40)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k40_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(41)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k41_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(42)             
        itl = itl + 1
        call bilinear_interpK_32gpt(k42_data, ig, pressure, p_ref_index, temperature, t_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
