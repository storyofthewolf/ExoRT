...
!
    ...
    do ik = 1,pverp 
      ...
      ! Interpolate to mixing ratio
      w_ref_index = kc_nvar
!      Find reference water vapor mixing ratio, h2o vmr
      do
        if (w_ref_index .le. 1) exit
        if ((wgrid(w_ref_index) .le. h2ovmr)) exit
        w_ref_index =  w_ref_index - 1
      enddo

!      ! if h2ovmr less than minimum of grid, force reference to minimum grid value
      if (h2ovmr .lt. wgrid(1)) then
        w_ref_index = 1
        h2ovmr = wgrid(w_ref_index)
      endif

!      ! if h2o vmr greater than maximum of grid, force reference to maximum grid value
!write(*,*) "h2ovmr", h2ovmr
      if (h2ovmr .ge. wgrid(kc_nvar)) then
        w_ref_index = kc_nvar
        h2ovmr = wgrid(w_ref_index)
      endif
      ...
      do ig = 1,ngauss_pts(1)      
        itl = itl + 1         
        call trilinear_interpK_16gpt(k01_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
      ...
      do ig = 1,ngauss_pts(2)    
        itl = itl + 1
        call trilinear_interpK_16gpt(k02_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(3)   
        itl = itl + 1
        call trilinear_interpK_16gpt(k03_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(4)          
        itl = itl + 1
        call trilinear_interpK_16gpt(k04_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col
      enddo   
      ...
      do ig = 1,ngauss_pts(5)   
        itl = itl + 1
        call trilinear_interpK_16gpt(k05_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(6)    
        itl = itl + 1
        call trilinear_interpK_16gpt(k06_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(7)		  
        itl = itl + 1
        call trilinear_interpK_16gpt(k07_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(8)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k08_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(9)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k09_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(10)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k10_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(11)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k11_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(12)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k12_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(13)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k13_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(14)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k14_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(15)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k15_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(16)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k16_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(17)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k17_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(18)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k18_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(19)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k19_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(20)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k20_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(21)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k21_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(22)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k22_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(23)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k23_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(24)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k24_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(25)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k25_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(26)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k26_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(27)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k27_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(28)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k28_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(29)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k29_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(30)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k30_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(31)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k31_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(32)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k32_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(33)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k33_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(34)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k34_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(35)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k35_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(36)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k36_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(37)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k37_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(38)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k38_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(39)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k39_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(40)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k40_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(41)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k41_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(42)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k42_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(43)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k43_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(44)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k44_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(45)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k45_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(46)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k46_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(47)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k47_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(48)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k48_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(49)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k49_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(50)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k50_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(51)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k51_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(52)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k52_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(53)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k53_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(54)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k54_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(55)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k55_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(56)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k56_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(57)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k57_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(58)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k58_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(59)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k59_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(60)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k60_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(61)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k61_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(62)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k62_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
      ...
      do ig = 1,ngauss_pts(63)             
        itl = itl + 1
        call trilinear_interpK_16gpt(k63_data, ig, pressure, p_ref_index, temperature, t_ref_index, &
                                     h2ovmr, w_ref_index, ans)
        tau_gas(itl,ik) = ans*u_col          
      enddo   
...
!      endif
      ...
      do iw = iwbeg,iwend     ! loop over bands
        call interph2o_cont(kh2oself, iw, temperature, t_ref_index_s, ans)
      ...
      itc=0
      do iw = iwbeg,iwend     ! loop over bands
        call interph2o_cont(kh2ofrgn, iw, temperature, t_ref_index_s, ans)
        do ig=1, ngauss_pts(iw)
          itc=itc+1
          tau_gas(itc,ik) = tau_gas(itc,ik) +ans*u_h2o*(pmid(ik)/1013.)*h2ovmr
        enddo
