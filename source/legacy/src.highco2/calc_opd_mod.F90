...
!      write(*,*) "-lev",ik,"n2,co2,h2o-"
!      write(*,*) "col",u_n2, u_co2, u_h2o
!      write(*,*) "vmr",n2vmr, co2vmr, h2ovmr
!      write(*,*) "mmr",qn2(ik), qco2(ik), qh2o(ik)
...
!        endif
      ...
      !!itc=0
      !!do iw = iwbeg,iwend    ! loop over bands      
      !!  call interpKself2(kco2cont, iw, temperature, t_ref_index_s, ans)
      !!    do ig=1, ngauss_pts(iw)
      !!      itc=itc+1
      !!      tau_gas(itc,ik) = tau_gas(itc,ik) +ans*u_co2*(pmid(ik)/1013.)*co2vmr  
      !!   enddo
      !!  enddo    ! close band loop      
